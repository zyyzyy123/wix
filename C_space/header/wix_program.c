...
int Read_etc(Etc_line * Ptrmsg,int Select){
	...
	if(Find==0){
		...
		for(i=1;i<=6;i++){		
			switch(i){
				case 1:
					Err_Echo(DEBUG_Read_etc,"%s:%s\tEXC_in\n",__func__,Buf);
					break;
				case 2:
					Buf[0]='\0';
					fscanf(fp,"%[^\t]s",Buf);
					Err_Echo(DEBUG_Read_etc,"%s:%s\tEXC_Out\n",__func__,Buf);
					Ptrmsg->EXC_Out=strdup(Buf);
					break;
				case 3:
					Buf[0]='\0';
					fscanf(fp,"%[^\t]s",Buf);
					Err_Echo(DEBUG_Read_etc,"%s:%s\tEXCp_Out\n",__func__,Buf);
					Ptrmsg->EXCp_Out=strdup(Buf);
					break;
				case 4:
					fscanf(fp,"%d",&Buf_int);
					Err_Echo(DEBUG_Read_etc,"%s:%d\tEXCp_Out_Num\n",__func__,Buf_int);
					Ptrmsg->EXCp_Out_num=Buf_int;
					break;
				case 5:
					fscanf(fp,"%d",&Buf_int);
					Err_Echo(DEBUG_Read_etc,"%s:%d\tDo\n",__func__,Buf_int);
					Ptrmsg->Do=Buf_int;
					break;
				case 6:
					fscanf(fp,"%d",&Buf_int);
					Err_Echo(DEBUG_Read_etc,"%s:%d\tPattern\n",__func__,Buf_int);
